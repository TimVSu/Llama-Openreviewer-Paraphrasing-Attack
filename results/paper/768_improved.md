...
## 1 Introduction
Beyond synonymy and paraphrasing, numerous NLP applications need to identify relationships between word meanings. For instance, given “Carlsen plays chess.” and the question “Which game does Carlsen play?”, successfully answering the question requires knowing that chess is a kind of game, or more generally, that chess entails game.
Lexical entailment has been conceptualized in earlier work as a word type relation (Turney and Mohammad, 2013), but we propose that entailment relations receive more effective definition when word meaning is demonstrated through contextual examples. Ignoring context is problematic since entailment might hold between some senses of the words, but not others. Consider the word game in two distinct contexts:
1. In NYC, the championship game took place.
2. The big game captured the hunters' interest.
...
## 5.2 CONTEXT-WN
In addition to CONTEXT-PPDB, we would like a test set for controlled analysis of lexical entailment in context. We aim to conduct a direct evaluation of our models' responsiveness to contexts indicating various word senses, and to measure the degree to which they recognize asymmetric entailment relations rather than semantic similarity.
...
## 8 Analysis
The experiments in Section 7 attest to the overall strength of our features. Further examination of the assumptions supporting our proposed context representations constitutes the primary objective of this section.
